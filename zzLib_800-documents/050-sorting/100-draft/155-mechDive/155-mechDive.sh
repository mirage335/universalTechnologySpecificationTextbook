...
_heading2 'Tension - Estimate - Eddy Current Brake - Peak kgF'
_t '$drum_RPM == Presume <30k to minimize requirements for drum strength and balance tolerance.

$drum_diameter == Presume <6in for convenient compactness, light weight, and low cost.

'
_e_ drum_RPM=30000
_e_ drum_diameter_cm=15

_i drum_rotationPerSecond='$(_clc "$drum_RPM / 60")'
_i drum_distancePerRotation_cm='$(_clc "2 * 3.14 * ($drum_diameter_cm * 0.5)")'
_i drum_speed_metersPerSecond='$(_clc "$drum_distancePerRotation_cm * 0.01 * $drum_rotationPerSecond")'
_o_ _safeEcho_newline 'drum_speed_metersPerSecond="$drum_speed_metersPerSecond"'

_t ' '
_ 'Usually calibrationMagnet_mult is approximate number of cm3 . Metal plate thickness (eg. 1/4inch) may also be relevant. Arbitrary anyway.'
_e_ calibrationMagnet_mult=0.5
_e_ calibrationMagnet_metersPerSecond=5
_e_ calibrationMagnet_kgF=0.005

_t ' '
_ 'Arbitrary approximate multiplier for high-force multi-slot (>>1cm thick) cable pull plates. Usually total magnet cm3 .'
_ 'Presumably constrained distance between drum and restraining pipe, magnetic field strength, density, etc, anyway.'
_e_ magnet_mult='$(_clc "1 * $drum_distancePerRotation_cm * 0.6")'

_t ' '
_e_ _clc '"($magnet_mult / $calibrationMagnet_mult) * ($calibrationMagnet_kgF / $calibrationMagnet_metersPerSecond) * $drum_speed_metersPerSecond"'
_o _safeEcho_newline 'magnet_kgF (magnet kilograms force) .'



_t 'Modeling, calibration, and optimization, of such an inexpensive device with such an intrinsic working principle, strongly favors experimentation instead of more extensive simulation and calculation. Estimates from experiments with permanent magnets opposing their own weight atop metal plates may be pessimistic.
...
_default_procedure() {
	...
	pdfunite "$scriptLib"/mechDive.pdf "$scriptLib"/"$1".pdf "$scriptAbsoluteFolder"/"$1".pdf
