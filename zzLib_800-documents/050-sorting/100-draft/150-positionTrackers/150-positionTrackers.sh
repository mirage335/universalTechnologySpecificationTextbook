...
_heading1 'positionTrackers'
_t 'Combined camera, &#39;lighthouse&#39;, ultrasonic, interferometric, speckle (highly precise structured light), magnetic, and/or ultrasonic, position tracking, for best possible accuracy and precision, at lowest cost.
...
_t 'Single (possibly flexible) PCB can integrate all the negligible cost widely available hardware for each plausible use case. Microcontrollers of the lowest cost and highest availability have sufficient analog and digital hardware.
...
_page ' ' ###
_heading2 'REFERENCE'
_t 'https://specklesense.media.mit.edu/text/zizka_specklesense_uist_2011.pdf
...
_heading2 'Estimate - Angular Diameter and Resolution - VR HOTAS positioning.'
_t 'Notably, if the ratios of observation diameter/distance and aperture wavelength/diameter are equal, then the observation angular diameter and aperture angular resolution will also be equal.
...
_e_ radian_arcSeconds=206265

_heading3 'Observation'
_e_ observation_distance_m=5
_e_ observation_diameter_mm=0.5

_ _e_ '_clc "2 * atan( ($observation_diameter_mm millimeter) / (2 * ($observation_distance_m meters) ) ) * $radian_arcSeconds"'
_e_ '_clc "($observation_diameter_mm * millimeter) / ($observation_distance_m * meter) * $radian_arcSeconds"'
_o _safeEcho_newline 'arc seconds.'


_heading3 'Aperture'
_e_ aperture_diameter_mm=10
_e_ aperture_wavelength_nm=1000

_e_ '_clc "(( $aperture_wavelength_nm nanometer) / ( $aperture_diameter_mm millimeter)) * $radian_arcSeconds"'
_o _safeEcho_newline 'arc seconds.'




_heading2 'Estimate - Angular Diameter and Resolution - Overlay ~10um .'

_e_ radian_arcSeconds=206265

_heading3 'Observation'
_e_ observation_distance_m=2.5
_e_ observation_diameter_mm=0.009125
...
_heading3 'Aperture'
_e_ aperture_diameter_mm=100
_e_ aperture_wavelength_nm=365
...
_default_procedure() {
	...
	pdfunite "$scriptLib"/"$1".pdf "$scriptLib"/positionTrackers.pdf "$scriptLib"/positionTrackers-errata.pdf "$scriptAbsoluteFolder"/"$1".pdf
